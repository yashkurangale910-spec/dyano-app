...
export default function ChatBot() {
    ...
    // Persist settings
    useEffect(() => {
        const storedPersonality = localStorage.getItem('sparke_personality');
        const storedDepth = localStorage.getItem('sparke_depth');
        const storedFramework = localStorage.getItem('sparke_framework');
    ...
    }, [personality, depth, selectedFramework]);
    ...
    useEffect(() => {
        const fetchPdfs = async () => {
            try {
                ...
                const res = await axios.get(`${API_URL}/pdf/list`, {
                    headers: { 'Authorization': `Bearer ${user.token}` }
                });
                if (res.data.success) setPdfList(res.data.documents || res.data.pdfs);
            } catch (err) {
    ...
    useEffect(() => {
        if ('webkitSpeechRecognition' in window || 'SpeechRecognition' in window) {
            ...
            // Safe access for browserCode
            const langObj = LANGUAGES.find(l => l.code === language) || LANGUAGES[0];
            recognitionRef.current.lang = langObj.browserCode;
    ...
    const handleChatSubmit = async (e) => {
        ...
        if (isQueryEmpty || status === 'loading') return;
        ...
        try {
            ...
            if (result && result.success) {
                ...
                // Simulate "Neural Synthesis" (typing effect)
                let currentText = "";
                ...
                const streamInterval = setInterval(() => {
                    if (i < words.length) {
                        ...
                        fetchProgress();
                        if (isSpeakEnabled) speak(fullText);
                    }
                }, 30); // Hyper-fast synaptic pulse
    ...
    return (
        <div className="fixed inset-0 z-20 flex bg-[#05010d] overflow-hidden">
                        ...
                        <div className="space-y-3 px-3">
                            <div className="space-y-2">
                                <div className="flex justify-between text-[8px] uppercase font-bold tracking-widest text-gray-500">
                                    ...
                                    <span className="text-cosmic-cyan">{progress?.stats?.averageScore || 0}%</span>
                                </div>
                                <div className="h-1 bg-white/5 rounded-full overflow-hidden">
                                    <motion.div initial={{ width: 0 }} animate={{ width: `${progress?.stats?.averageScore || 0}%` }} className="h-full bg-gradient-to-r from-cosmic-cyan to-cosmic-purple" />
                                </div>
                            </div>
                            <div className="grid grid-cols-2 gap-2">
                                <div className="p-3 bg-white/5 border border-white/5 rounded-xl">
                                    <div className="text-[7px] text-gray-600 uppercase font-bold mb-1">Solved</div>
                                    <div className="text-sm font-bold text-white">{progress?.stats?.totalQuizzesTaken || 0}</div>
                                </div>
                                <div className="p-3 bg-white/5 border border-white/5 rounded-xl">
                                    <div className="text-[7px] text-gray-600 uppercase font-bold mb-1">Graded</div>
                                    <div className="text-sm font-bold text-white">{progress?.stats?.totalEssaysGraded || 0}</div>
                                </div>
                            </div>
                        </div>
                    </div>
            ...
            <main className="flex-1 flex flex-col relative overflow-hidden h-screen bg-black/10">
                <header className="h-16 px-6 border-b border-white/5 flex items-center justify-between bg-black/40 backdrop-blur-xl shrink-0 z-10">
                    ...
                    <div className="flex items-center gap-3">
                        ...
                        <button onClick={clearChat} className="w-8 h-8 rounded-lg flex items-center justify-center bg-white/5 text-gray-500 hover:text-red-400 transition">
                            <RefreshCw size={14} />
                        </button>
                    </div>
                </header>
                            ...
                            {activeTab === 'chat' && (
                                <motion.div key="chat" initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-10">
                                    {messages.map((msg, idx) => (
                                        ...
                                        >
                                            <div className={`w-10 h-10 md:w-11 md:h-11 rounded-xl flex items-center justify-center shrink-0 shadow-lg border backdrop-blur-md transition-all duration-500 ${msg.role === 'user' ? 'bg-cosmic-pink/20 text-cosmic-pink border-cosmic-pink/30' : `bg-cosmic-${PERSONALITY_THEMES[personality]?.primary}/20 text-cosmic-${PERSONALITY_THEMES[personality]?.primary} border-cosmic-${PERSONALITY_THEMES[personality]?.primary}/30`}`}>
                                            ...
                                            <div className={`max-w-[85%] md:max-w-[80%] ${msg.role === 'user' ? 'text-right' : ''}`}>
                                                <div className={`p-5 md:p-6 rounded-2xl text-[13px] leading-relaxed shadow-deep border transition-colors duration-1000 group/msg relative ${msg.role === 'user'
                                                    ...
                                                    <div className={`whitespace-pre-wrap selection:bg-cosmic-cyan/30 ${msg.isStreaming ? 'relative' : ''}`}>
                                                        {typeof msg.content === 'string' ? msg.content : msg.content?.text || msg.content?.response}
                                                        {msg.isStreaming && (
                                                            <motion.span
                                                                animate={{ opacity: [0.2, 1, 0.2] }}
                                                                transition={{ duration: 0.8, repeat: Infinity }}
                                                                className="inline-block w-2 h-4 ml-1 bg-cosmic-cyan shadow-glow-cyan align-middle rounded-sm"
                                                            />
                                                        )}
                                                    </div>

                                                    {/* Citations (NEW) */}
                                                    {(msg.content?.citations || msg.citations) && (msg.content?.citations?.length > 0 || msg.citations?.length > 0) && !msg.isStreaming && (
                                                        <div className="mt-4 pt-4 border-t border-white/5 space-y-2 animate-in fade-in slide-in-from-top-2 duration-700">
                                                            <div className="text-[9px] font-black text-gray-500 uppercase tracking-widest flex items-center gap-2">
                                                                <BookOpen size={10} /> Neural Citations
                                                            </div>
                                                            <div className="flex flex-wrap gap-2">
                                                                {(msg.content?.citations || msg.citations).map((cite, ci) => (
                                                                    <div key={ci} className="px-2 py-1 bg-white/5 border border-white/5 rounded text-[8px] text-gray-400 font-mono hover:bg-white/10 transition-colors cursor-help" title={cite.snippet}>
                                    ...
                                    {status === 'loading' && (
                                        <div className="flex gap-4 md:gap-6">
                                            ...
                                            <div className="flex items-center gap-4 bg-white/5 border border-white/10 px-6 py-4 rounded-3xl rounded-tl-none">
                                                ...
                                                <span className="text-[9px] uppercase font-bold tracking-[0.4em] text-cosmic-cyan">{t('tutor.processing') || 'Processing'}</span>
                                            </div>
                                        </div>
                                    )}
            ...
            <AnimatePresence>
                {showConfig && (
                    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 bg-black/95 backdrop-blur-2xl z-[100] p-6 md:p-12 overflow-y-auto custom-scrollbar">
                        <div className="max-w-2xl mx-auto space-y-12">
                            ...
                            <section className="space-y-6">
                                <label className="text-[10px] uppercase tracking-[0.4em] font-bold text-cosmic-cyan">Neural Grounding (PDFs)</label>
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                    {pdfList.map(pdf => (
                                        <button
                                            key={pdf._id}
                                            onClick={() => setSelectedPdf(selectedPdf?._id === pdf._id ? null : pdf)}
                                            className={`p-4 rounded-2xl border text-left transition-all ${selectedPdf?._id === pdf._id
                                                ? 'border-cosmic-cyan bg-cosmic-cyan/10'
                                                : 'border-white/5 bg-white/5 hover:bg-white/[0.08]'
                                                }`}
                                        >
                                            <div className="flex items-center gap-2 mb-2">
                                                <FileText size={16} className={selectedPdf?._id === pdf._id ? 'text-cosmic-cyan' : 'text-gray-500'} />
                                                <div className={`text-[10px] font-bold truncate ${selectedPdf?._id === pdf._id ? 'text-white' : 'text-gray-400'}`}>{pdf.fileName}</div>
                                            </div>
                                        </button>
                                    ))}
                                    {pdfList.length === 0 && <div className="text-gray-500 text-xs italic">No documents available. Upload via PDF Sync.</div>}
                                </div>

                                {selectedPdf && (
                                    <div className="flex items-center justify-between p-4 bg-white/5 border border-white/10 rounded-2xl mt-4">
                                        <div>
                                            <p className="text-[10px] font-bold text-white uppercase tracking-wider">Deep Context Mode</p>
                                            <p className="text-[8px] text-gray-500">Enable neural RAG for precise document analysis</p>
                                        </div>
                                        <button
                                            onClick={() => setIsDeepContext(!isDeepContext)}
                                            className={`w-12 h-6 rounded-full transition-all relative ${isDeepContext ? 'bg-cosmic-cyan' : 'bg-gray-800'}`}
                                        >
                                            <motion.div
                                                animate={{ x: isDeepContext ? 24 : 4 }}
                                    ...
                                    {PERSONALITIES.map(p => (
                                        <button key={p.id} onClick={() => setPersonality(p.id)} className={`p-4 rounded-2xl border text-left transition-all ${personality === p.id ? 'border-cosmic-cyan bg-cosmic-cyan/10' : 'border-white/5 bg-white/5'}`}>
                                            <div className="flex items-center gap-3 mb-2">
                                                ...
                                                <div className="text-[10px] font-bold text-white">{p.label}</div>
                                            </div>
                                            <div className="text-[8px] text-gray-500 uppercase leading-relaxed">{p.desc}</div>
                                        </button>
                                    ))}
                                </div>
                            </section>
                                ...
                                <div className="grid grid-cols-3 md:grid-cols-5 gap-3">
                                    {LANGUAGES.map(l => (
                                        <button
                                            ...
                                            className={`p-3 rounded-xl border text-center transition-all ${language === l.code ? 'border-cosmic-cyan bg-cosmic-cyan/10' : 'border-white/5 bg-white/[0.03] hover:bg-white/10'}`}
                                        >
                                            <div className="text-xl mb-1">{l.flag}</div>
                                            <div className={`text-[9px] font-bold uppercase ${language === l.code ? 'text-white' : 'text-gray-500'}`}>{l.label}</div>
                                        </button>
                                    ))}
                                </div>
                            </section>

                            <button onClick={() => setShowConfig(false)} className="w-full py-5 bg-cosmic-cyan text-black rounded-2xl text-[10px] uppercase font-bold tracking-widest hover:scale-[1.02] transition-transform">Apply Synaptic Weights</button>
                        </div>
                    </motion.div>
                )}
            </AnimatePresence>
